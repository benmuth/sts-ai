...
void BattleContext::init(const GameContext &gc, MonsterEncounter encounterToInit, bool burningElite) {
    ...
    // TODO: figure out if hasRelic and relics.has are both necessary 
...
void BattleContext::initRelics(RelicContainer relics, sts::Room curRoom, sts::Room prevRoom, int curHp, int maxHp) {
    player.relicBits0 = relics.relicBits0;
    player.relicBits1 = relics.relicBits1;
...
//                p.cardDrawPerTurn++;
    ...
    // TODO: see if gc.hasRelic is equivalent to relics.has
    ...
    //     p.buff<PS::STRENGTH>(3);
    // }
    if (relics.has(R::MERCURY_HOURGLASS)) {
        addToBot( Actions::DamageAllEnemy(3) );
    }

    if (relics.has(R::RED_SKULL) && curHp <= maxHp / 2) {
        p.buff<PS::STRENGTH>(3);
    }
