...
void CardManager::init(const sts::Deck deck, BattleContext &bc) {

//    masterDeckSize = gc.deck.size();
    nextUniqueCardId = deck.size();
    handPainCount = false;
    handNormalityCount = false;
    strikeCount = 0;

    fixed_list<int, Deck::MAX_SIZE> idxs(deck.size());
    for (int i = 0; i < idxs.size(); ++i) {
        idxs[i] = i;
    }
    java::Collections::shuffle(idxs.begin(), idxs.end(), java::Random(bc.shuffleRng.randomLong()));

    drawPile.resize(deck.size());
    discardPile.clear();
    exhaustPile.clear();
    ...
    for (int i = 0; i < deck.size(); ++i) {
        ...
        const auto &deckCard = deck.cards[deckIdx];
//        if (deckCard.isStrikeCard()) { ++strikeCount; } do at createDeckCardInstanceInDrawPile

        bool isBottled = std::find(deck.bottleIdxs.begin(), deck.bottleIdxs.end(), deckIdx) != deck.bottleIdxs.end();
