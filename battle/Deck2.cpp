...
void Deck::obtain(Card card, int count, RelicContainer& relics) {
    ...
    switch (type) {
        case CardType::ATTACK:
            ...
            if (relics.has(RelicId::MOLTEN_EGG)) {
                card.upgrade();
        ...
        case CardType::SKILL:
            ...
            if (relics.has(RelicId::TOXIC_EGG)) {
                card.upgrade();
            } else if (card.canUpgrade()) {
                upgradeableCount += count;
            }
        ...
        case CardType::POWER:
            ...
            if (relics.has(RelicId::FROZEN_EGG)) {
                card.upgrade();
            } else if (card.canUpgrade()) {
                upgradeableCount += count;
            }
        ...
        case CardType::CURSE:
            if (relics.has(RelicId::OMAMORI)) {
                int &omamoriCount = relics.getRelicValueRef(RelicId::OMAMORI);
                if (omamoriCount > 0) {
    ...
    // if (relics.has(RelicId::CERAMIC_FISH)) { // todo check if ceramic fish activates before omamori
    //     player.gainGold(bc, 9 * count);
    // }
